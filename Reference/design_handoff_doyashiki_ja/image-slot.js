...
(() => {
  ...
  const stylesheet =
    ...
    // Touch devices have no hover — keep controls visible when filled so the
    // image is still replaceable. And controls="top" pins them INSIDE the slot
    // (top-center) for full-bleed slots whose top:100% controls fall off-screen.
    '@media (hover:none){:host([data-filled][data-editable]) .ctl{opacity:1;pointer-events:auto}}' +
    ':host([controls="top"]) .ctl{top:12px;bottom:auto;padding-top:0}' +
